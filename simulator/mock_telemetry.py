...
# Lake coordinates (lat, lon) for Open-Meteo queries
LAKES = [
...
# Per-lake state for smooth simulation
lake_state = {}
for lake in LAKES:
    lake_state[lake["id"]] = {
        "water_level": random.uniform(10, 80),
        "drift": random.uniform(0.1, 0.5),     # cm per tick upward drift
        "phase": random.uniform(0, 2 * math.pi),
    }

# Cache for Open-Meteo data (refresh every 30 minutes)
weather_cache = {}
CACHE_TTL = 1800
...
def main():
    ...
    print(f"[SIM] Interval: {INTERVAL}s | Lakes: {len(LAKES)}")
    ...
    while True:
        ...
        lakes_this_tick = random.sample(LAKES, min(3, len(LAKES)))
